...
def upgrade() -> None:
    # Drop the existing unique index on n8n_workflow_id
    ...
    op.create_unique_constraint(
        "unique_user_n8n_workflow", "workflow_configs", ["user_id", "n8n_workflow_id"]
    )

    # Recreate the index (without unique constraint)
    op.create_index(
        op.f("ix_workflow_configs_n8n_workflow_id"),
        "workflow_configs",
        ["n8n_workflow_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "unique_user_n8n_workflow", table_name="workflow_configs", type_="unique"
    )

    # Recreate the global unique index on n8n_workflow_id
    op.drop_index("ix_workflow_configs_n8n_workflow_id", table_name="workflow_configs")
    op.create_index(
        op.f("ix_workflow_configs_n8n_workflow_id"),
        "workflow_configs",
